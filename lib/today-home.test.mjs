...
test("getTodayTaskPreview limits task noise and reports hidden count", () => {
  ...
  const preview = todayHome.getTodayTaskPreview(tasks, 6)

  assert.equal(preview.visibleTasks.length, 6)
  assert.equal(preview.hiddenTaskCount, 3)
})
